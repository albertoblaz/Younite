...
(function() {
  var Connector, Delegate, Storage, signupRight, signupWrong, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7,
  ...
  Connector = (function() {
    ...
    Connector.prototype.downloadFriends = function() {
      var arr, fid, iter, nmax, q, _i, _len, _ref3, _results,
        _this = this;
      iter = 0;
      nmax = App.Me.friends.length;
      arr = [];
      _ref3 = App.Me.friends;
      _results = [];
      for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
        fid = _ref3[_i];
        q = $.get("/users/" + fid);
  ...
  App.Party = (function(_super) {
    __extends(Party, _super);

    Party.fields("id", "name", "description", "date", "idDisco", "site", "users", "price");

    function Party() {
      Party.__super__.constructor.apply(this, arguments);
    }

    Party.prototype.attend = function() {
      this.users.push(App.Me);
      this.updateAttributes({
        users: this.users
      });
      return this;
    ...
    Party.prototype.recommend = function() {};

    Party.prototype.validate = function() {
      if (!this.id) {
        "id is required";
      }
      if (!this.name) {
  ...
  App.PartyView = (function(_super) {
    __extends(PartyView, _super);

    PartyView.prototype.template_url = "app/templates/PartyTemplate.mustache";

    PartyView.prototype.events = {
      "tap .button": "onAttend",
      "tap li": "onTap"
    };

    function PartyView() {
      PartyView.__super__.constructor.apply(this, arguments);
    }

    PartyView.prototype.onTap = function(event) {
      return Monocle.Route.navigate("/parties/" + this.model.id);
    };

    PartyView.prototype.onAttend = function(event) {
      event.preventDefault();
      event.stopPropagation();
      return this.model.attend();
    };
  ...
  App.PartiesController = (function(_super) {
    ...
    PartiesController.prototype.elements = {
      "#following ul": "fol",
      "#recommended ul": "rec",
      "#count-fol": "countfol",
      "#count-rec": "countrec"
    ...
    PartiesController.prototype.events = {
      "tap #search": "onSearch",
      "load #parties": "onLoad"
    };
    ...
    function PartiesController() {
      ...
      this.bindDelete = __bind(this.bindDelete, this);
      this.bindChange = __bind(this.bindChange, this);
      this.bindCreate = __bind(this.bindCreate, this);
      var p,
        _this = this;
      PartiesController.__super__.constructor.apply(this, arguments);
      App.Party.bind("create", this.bindCreate);
      App.Party.bind("change", this.bindChange);
      App.Party.bind("delete", this.bindDelete);
      App.Party.bind("error", this.bindError);
      p = $.getJSON("/events");
      p.done(function(parties) {
        ...
        for (_i = 0, _len = parties.length; _i < _len; _i++) {
          ...
          _results.push(App.Party.create(p));
        }
        return _results;
      });
    ...
    PartiesController.prototype.onLoad = function(event) {
      return this.render();
    };

    PartiesController.prototype.onSearch = function(event) {};

    PartiesController.prototype.render = function() {
      var num;
      num = this.fol.children().length;
      if (num) {
        this.countfol.css("display", "inline").text(num);
      } else {
        this.countfol.hide();
      }
      num = this.rec.children().length;
      if (num) {
        return this.countrec.css("display", "inline").text(num);
      } else {
        return this.countrec.hide();
      }
    };

    PartiesController.prototype.bindCreate = function(party) {
      var view;
      view = new App.PartyView({
        model: party
      });
      view.container = this.fol;
      return view.append(party);
    };

    PartiesController.prototype.bindChange = function(party) {};

    PartiesController.prototype.bindDelete = function(party) {
      App.Utils.showError("Deleting the party " + party.name);
      throw "You've deleted " + party.name + "!, " + party;
    };

    PartiesController.prototype.bindError = function(party) {
      return App.Utils.showError("Site error spotted, " + party.name + "!");
    };
  ...
  App.SiteView = (function(_super) {
    ...
    SiteView.prototype.removeSiteView = function(model) {
      ...
      if (this.model.equal(model)) {
        this.el.remove();
        return App.Site.unbind("removeSiteView", this.removeSiteView);
      }
    };
  ...
  App.SitesController = (function(_super) {
    ...
    SitesController.prototype.sitesChanged = [];
    ...
    SitesController.prototype.onLoad = function(event) {
      var site, _i, _len, _ref4;
      _ref4 = this.sitesChanged;
      for (_i = 0, _len = _ref4.length; _i < _len; _i++) {
        site = _ref4[_i];
        site.trigger("removeSiteView");
        this.bindCreate(site);
      }
      return this.sitesChanged = [];
    };
    ...
    SitesController.prototype.bindChange = function(site) {
      if (!this.sitesChanged.contains(site)) {
        return this.sitesChanged.push(site);
      }
    };
  ...
  App.MapController = (function(_super) {
    ...
    function MapController() {
      _ref4 = MapController.__super__.constructor.apply(this, arguments);
      return _ref4;
    }
  ...
  App.SettingsController = (function(_super) {
    ...
    function SettingsController() {
      _ref5 = SettingsController.__super__.constructor.apply(this, arguments);
      return _ref5;
    }
  ...
  App.HelpController = (function(_super) {
    ...
    function HelpController() {
      _ref6 = HelpController.__super__.constructor.apply(this, arguments);
      return _ref6;
    }
  ...
  App.UserFriendController = (function(_super) {
    ...
    UserFriendController.prototype.renderFriends = function(user) {
      var friend, method, view, _i, _len, _ref7;
  ...
  App.PartyController = (function(_super) {
    ...
    PartyController.prototype.elements = {
      "#name": "name",
      "#picture": "picture",
    ...
    PartyController.prototype.events = {
      "tap #btn-attend": "onAttend",
      "tap #btn-recommend": "onRecommend"
    };

    PartyController.prototype.currentParty = null;

    function PartyController() {
      PartyController.__super__.constructor.apply(this, arguments);
      this.routes({
        "/parties/:id": this.loadProfile
      });
      Monocle.Route.listen();
    }

    PartyController.prototype.onAttend = function(event) {
      return this.currentSite.attend();
    };

    PartyController.prototype.onRecommend = function(event) {
      var _this = this;
      return Lungo.Notification.confirm({
        icon: "thumbs-up",
        title: "Recommend?",
        description: "Do you want to recommend " + this.currentParty.name + " to your friends?",
        accept: {
          icon: "check",
          label: "Recommend",
          callback: function() {
            return _this.currentParty.recommend();
          }
        },
        cancel: {
          icon: "multiply",
    ...
    PartyController.prototype.loadProfile = function(params) {
      var party;
      party = App.Party.findBy("id", params.id);
      if (!party) {
        party = this.download(params.id);
      }
      this.render(party);
      this.currentParty = party;
      return Lungo.Router.section("#party");
    };

    PartyController.prototype.render = function(party) {
      this.renderInfo(party);
      return this.renderFriends(party);
    };

    PartyController.prototype.renderInfo = function(party) {
      this.name.text(party.name);
      this.description.text(party.description);
      return this.picture[0].src = party.picture;
    };

    PartyController.prototype.renderFriends = function(party) {};

    PartyController.prototype.download = function(id) {
      var data;
      console.log("download party " + id);
      data = App.Connector.parties.get(id);
      return App.Party.create(data);
    };
  ...
  App.SiteController = (function(_super) {
    ...
    SiteController.prototype.renderComments = function(site) {
      var c, view, _i, _j, _len, _len1, _ref7, _ref8, _results;
      _ref7 = this.comments.children(".arrow");
      for (_i = 0, _len = _ref7.length; _i < _len; _i++) {
        c = _ref7[_i];
        c.remove();
      }
  ...
  App.Activity = (function(_super) {
    ...
    function Activity() {
      _ref7 = Activity.__super__.constructor.apply(this, arguments);
      return _ref7;
    }
